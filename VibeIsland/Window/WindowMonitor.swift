...
class WindowMonitor {
    ...
    /// 日志器
    private let logger = Logger(subsystem: "com.vibeisland", category: "WindowMonitor")
    ...
    /// 上次检测到的窗口标题（每个窗口单独跟踪）
    private var lastWindowTitles: [String: String] = [:]
    ...
    func checkActiveWindow() {
        ...
        guard let frontmostApp = NSWorkspace.shared.frontmostApplication else {
            logger.warning("无法获取前端应用")
            return
        }

        let bundleId = frontmostApp.bundleIdentifier ?? ""
        logger.debug("检查窗口: \(frontmostApp.localizedName ?? bundleId)")

        // 检查是否是终端应用（可能正在运行 Claude Code）
        if terminalBundleIds.contains(bundleId) {
            checkTerminalWindow(frontmostApp)
        } else {
            // 即使前端不是终端，也检查所有运行的终端应用
            checkAllTerminalApps()
        }
    }

    /// 手动触发检测（用于调试）
    func manualCheck() {
        logger.info("手动触发窗口检测")
        checkAllTerminalApps()
    }

    /// 检查所有运行的终端应用
    private func checkAllTerminalApps() {
        for bundleId in terminalBundleIds {
            let runningApps = NSRunningApplication.runningApplications(withBundleIdentifier: bundleId)
            for app in runningApps {
                checkTerminalWindow(app)
            }
        }
    }
    ...
    private func checkTerminalWindow(_ app: NSRunningApplication) {
        ...
        let windowTitles = getAllWindowTitles(for: app)
        let appIdentifier = app.bundleIdentifier ?? "unknown"
        ...
        for windowTitle in windowTitles {
            ...
            // 检查是否与上次相同
            let key = "\(appIdentifier)-\(windowTitle.hash)"
            if lastWindowTitles[key] == windowTitle {
                continue
            }

            lastWindowTitles[key] = windowTitle
    ...
    /// 使用 Accessibility API 获取所有窗口标题
    private func getAllWindowTitles(for app: NSRunningApplication) -> [String] {
        ...
        guard windowsResult == .success, let windowsArray = windows as? [AXUIElement] else {
            // 如果获取所有窗口失败，尝试获取焦点窗口
            return [getFocusedWindowTitle(for: app)]
        }
    ...
    /// 获取焦点窗口标题（作为备用）
    private func getFocusedWindowTitle(for app: NSRunningApplication) -> String {
        let pid = app.processIdentifier
        let appRef = AXUIElementCreateApplication(pid)
